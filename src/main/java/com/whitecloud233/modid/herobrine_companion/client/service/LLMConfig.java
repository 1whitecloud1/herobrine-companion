...
public class LLMConfig {
    ...
    public static boolean isWebLookupEnabled() {
        ensureLoaded();
        // 安全版构建（联网查找与电脑 CMD / JVM 代码注入同组被排除）：开关强制关闭。
    ...
    /**
     * 按本次实际主模型设置计算上下文窗口，避免全局云端 Provider 影响本地路由。
     *
     * <p>真实值由 {@link LLMContextWindow} 提供（按 endpoint+model 缓存并落盘）；
     * 本地回环在探测未返回时用启动器实际下发的 {@code -c} 参数兜底。</p>
     */
    ...
    public static int getEffectiveConversationHistoryTokenBudget() {
        int contextWindow = getContextWindowTokens();
        return clampInt(contextWindow / 3, 4_096, 16_384, 8_192);
    }
