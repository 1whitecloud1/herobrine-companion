...
import com.whitecloud233.modid.herobrine_companion.client.fight.network.CPacketCollapseFinished;
...
public class FakeCrashScreen extends Screen {
    ...
    // 【核心升级】：玻璃碎片改为极其不规则的尖锐三角形
    private static class GlassShard {
        float x, y, vx, vy;
        float angle, angularVel;
        float[] ptsX = new float[3]; // 3个顶点组成尖锐的三角形
    ...
    protected void init() {
        ...
        // 【核心新增】：使用翻译键加载文本内容，并直接提取为字符串以便做打字机效果
    ...
    private void generateExplosionShards() {
        ...
        for (int i = 0; i < 150; i++) {
            ...
            float speed = 15 + rand.nextFloat() * 45; // 增加爆炸初速度
            float vx = (float) Math.cos(angle) * speed;
            float vy = (float) Math.sin(angle) * speed;

            // 【核心修改】：碎片放大了数倍，最小 10，最大达到 45 像素
    ...
    public void tick() {
        ...
        if (ticksOpen == 200 && mc.player != null) mc.player.playSound(SoundEvents.UI_BUTTON_CLICK.get(), 1.0F, 0.5F);
        ...
        if (ticksOpen == 490 && mc.player != null) {
            ...
            mc.player.playSound(SoundEvents.GENERIC_EXPLODE, 1.0F, 0.5F);
    ...
    private void playTypeSound(Minecraft mc) {
        if (mc.player != null) mc.player.playSound(SoundEvents.NOTE_BLOCK_HAT.get(), 1.0f, 1.5f);
    }
    ...
    private void drawShardTriangle(GuiGraphics graphics, float[] ptsX, float[] ptsY, int color) {
        ...
        // 使用 TRIANGLES 模式绘制真正的锐角多边形
        bufferbuilder.begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);

        for (int i = 0; i < 3; i++) {
            bufferbuilder.vertex(pose, ptsX[i], ptsY[i], 0).color(r, g, b, a).endVertex();
        }
