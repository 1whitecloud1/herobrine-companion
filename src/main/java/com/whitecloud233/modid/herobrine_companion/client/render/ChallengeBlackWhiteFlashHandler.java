...
public final class ChallengeBlackWhiteFlashHandler {
    ...
    private static final ResourceLocation POST_CHAIN_LOCATION =
            new ResourceLocation(HerobrineCompanion.MODID, "shaders/post/challenge_bw_flash.json");
    ...
    public static void onRenderGuiPre(RenderGuiEvent.Pre event) {
        ...
        Minecraft minecraft = Minecraft.getInstance();
        RenderTarget mainTarget = minecraft.getMainRenderTarget();
        float remaining = Mth.clamp((flashTicks - event.getPartialTick()) / FLASH_TICKS, 0.0F, 1.0F);
    ...
    public static void onRenderGuiPost(RenderGuiEvent.Post event) {
        ...
        graphics.fill(0, 0, event.getWindow().getGuiScaledWidth(), event.getWindow().getGuiScaledHeight(), color);
    }
}
