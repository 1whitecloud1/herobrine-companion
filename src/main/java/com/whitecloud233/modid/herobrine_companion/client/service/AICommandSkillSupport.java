...
final class AICommandSkillSupport {
    ...
    private static String buildMinecraftCommandSkillDescription() {
        return "MCP-style structured Minecraft Java Edition command catalog for Herobrine. Pick one action enum and fill parameter slots; never invent raw /commands for covered actions. "
                + "Project runtime is Minecraft Java 1.20.1, so newer wiki commands may be documented here but can still be rejected by the 1.20.1 dispatcher. "
    ...
    private static JsonObject createMinecraftCommandSkillInputSchema() {
        ...
        addStringProperty(properties, "item_id", "For give_item or clear_inventory. Registered item id: minecraft:diamond, diamond, or modid:item_id for installed mods (e.g. tacz:ak47); current-client localized display names may be resolved.");
