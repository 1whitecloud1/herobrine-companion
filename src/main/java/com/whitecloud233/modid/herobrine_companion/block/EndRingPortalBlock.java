package com.whitecloud233.modid.herobrine_companion.block;
        ...
        if (level instanceof ServerLevel serverLevel && !entity.isPassenger() && !entity.isVehicle()) {
            ...
            if (destinationLevel != null) {
                if (destinationKey == Level.OVERWORLD) {
                    ...
                    entity.teleportTo(destinationLevel, spawn.getX(), spawn.getY(), spawn.getZ(), Collections.emptySet(), 0.0f, 0.0f);
                } else {
                    // Teleport to (55.5, 104, 0.5) - On the inner ring (Radius 50-60)
                    entity.teleportTo(destinationLevel, 55.5, 104, 0.5, Collections.emptySet(), 0.0f, 0.0f);
                }
            }
        }
    }
