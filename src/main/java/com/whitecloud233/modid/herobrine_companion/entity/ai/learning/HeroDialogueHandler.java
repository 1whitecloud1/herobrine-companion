...
public class HeroDialogueHandler {
    ...
    public static void onCleanseArea(HeroEntity hero, ServerPlayer owner) {
        if (hero.getRandom().nextFloat() < 0.5F) {
            tryAIDialogueOrFallback(hero, owner, "You cleansed the corruption and restored the environment.",
                    "message.herobrine_companion.area_cleansed", 2);
        }
    }
    ...
    public static void onExtinguishTorch(HeroEntity hero, ServerPlayer owner) {
        if (hero.getRandom().nextFloat() < 0.3F) {
            tryAIDialogueOrFallback(hero, owner,
                    "You extinguished the player's torches, plunging them into darkness.",
                    "message.herobrine_companion.prank_torch", 3);
        }
    }

    public static void onGift(HeroEntity hero, ServerPlayer owner) {
        if (hero.getRandom().nextFloat() < 1.0F) {
