...
public class HeroBattleStanceGoal extends Goal {
    ...
    private static final int DEFAULT_ACTION_DURATION = 10;
    ...
    private static boolean isNonVanillaHostile(LivingEntity candidate) {
        ResourceLocation entityId = candidate == null ? null : ForgeRegistries.ENTITY_TYPES.getKey(candidate.getType());
    ...
    private int getActionDuration(int action) {
        ...
        if (stack.getItem() instanceof PoemOfTheEndItem poem) {
            return switch (poem.getMode(stack)) {
                case PoemOfTheEndItem.MODE_VOID_SHATTER -> action == HeroEntity.BATTLE_ACTION_LIGHT_COMBO_1 ? 7 : 8;
                case PoemOfTheEndItem.MODE_REALM_BREAKER, PoemOfTheEndItem.MODE_THUNDER_CALL -> action == HeroEntity.BATTLE_ACTION_LIGHT_COMBO_1 ? 14 : 16;
                default -> action == HeroEntity.BATTLE_ACTION_LIGHT_COMBO_1 ? 10 : 12;
            };
        }
        return action == HeroEntity.BATTLE_ACTION_LIGHT_COMBO_1 ? 8 : 10;
    }
    ...
    private int getHitFrame(int action) {
        ...
        if (stack.getItem() instanceof PoemOfTheEndItem poem) {
            return switch (poem.getMode(stack)) {
                ...
                case PoemOfTheEndItem.MODE_REALM_BREAKER, PoemOfTheEndItem.MODE_THUNDER_CALL -> action == HeroEntity.BATTLE_ACTION_LIGHT_COMBO_1 ? 6 : 7;
