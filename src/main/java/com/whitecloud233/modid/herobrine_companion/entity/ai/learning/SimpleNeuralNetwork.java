...
public class SimpleNeuralNetwork {
    ...
    private void decayInputs() {
        ...
        // 现在改为 0.005f，需要 200次 tick (1000秒 = 16分钟) 才会归零
    ...
    public void save(CompoundTag tag) {
        // [修改] 直接保存到传入的 tag，而不是创建一个子 tag "HeroNeuralNetwork"
        // 因为上层 HeroBrain 已经为每个玩家创建了独立的 CompoundTag
    ...
    public void load(CompoundTag tag) {
        // [修改] 直接从传入的 tag 读取
        if (tag.contains("Violence")) { // 简单检查是否包含数据
    ...
    public String getDebugInfo() {
        return String.format("St:%s | V:%.2f E:%.2f M:%.2f N:%.2f | R:%.2f A:%.2f Arr:%.2f",
                currentState.name().substring(0, 3), violenceScore, entropyScore, metaScore, nostalgiaScore, respectWeight, annoyanceWeight, arroganceWeight);
    }
}
