...
public final class HeroCombatWeaponHelper {
   ...
   public static boolean isGunLikeWeapon(ItemStack stack) {
      if (stack != null && !stack.isEmpty()) {
         ResourceLocation itemId = BuiltInRegistries.ITEM.getKey(stack.getItem());
         String itemPath = itemId != null ? itemId.toString().toLowerCase(Locale.ROOT) : "";
         return itemPath.contains("gun") || itemPath.contains("firearm") || itemPath.contains("rifle") || itemPath.contains("pistol") || itemPath.contains("revolver") || itemPath.contains("musket") || itemPath.contains("shotgun") || itemPath.contains("sniper") || itemPath.contains("launcher") || itemPath.contains("cannon") || itemPath.contains("blaster");
      } else {
         return false;
      }
   }
   ...
   public static int getChargeTicks(ItemStack stack) {
      return getChargeTicks(stack, (LivingEntity)null);
   }
   ...
   public static int getChargeTicks(ItemStack stack, LivingEntity shooter) {
      if (isCrossbowWeapon(stack)) {
         ...
         UseAnim useAnim = stack.getUseAnimation();
         return useAnim == UseAnim.BOW ? 20 : 12;
      }
   }
   ...
   public static int getPostShotCooldown(ItemStack stack) {
      if (isCrossbowWeapon(stack)) {
         return 30;
      } else if (stack != null && stack.getUseAnimation() == UseAnim.BOW) {
         return 20;
      } else {
         return isGunLikeWeapon(stack) ? 14 : 30;
      }
   }
   ...
   public static float getPreferredAttackRadius(ItemStack stack) {
      if (isGunLikeWeapon(stack)) {
         return 18.0F;
      } else if (isCrossbowWeapon(stack)) {
         return 16.0F;
      } else {
         return stack != null && stack.getUseAnimation() == UseAnim.BOW ? 15.0F : 14.0F;
      }
   }
   ...
   public static boolean fireRangedWeaponAtTarget(HeroEntity hero, LivingEntity target, ItemStack weaponStack, int chargeTicks) {
      if (hero != null && target != null && weaponStack != null && !weaponStack.isEmpty()) {
         if (!isCrossbowWeapon(weaponStack) && weaponStack.getUseAnimation() != UseAnim.CROSSBOW) {
            return weaponStack.getUseAnimation() != UseAnim.BOW && !(weaponStack.getItem() instanceof BowItem) ? fireVirtualProjectileAtTarget(hero, target, weaponStack, Math.max(chargeTicks, 12), 2.6F, 4.5F, SoundEvents.CROSSBOW_SHOOT) : fireBowAtTarget(hero, target, weaponStack, chargeTicks);
         } else {
   ...
   public static boolean fireBowAtTarget(HeroEntity hero, LivingEntity target, ItemStack bowStack, int chargeTicks) {
      if (hero != null && target != null && bowStack != null && !bowStack.isEmpty() && !hero.level().isClientSide) {
         ...
         float power = BowItem.getPowerForTime(Math.max(chargeTicks, 20));
         if (power < 0.1F) {
            return false;
         } else {
            AbstractArrow arrow = arrowItem.createArrow(hero.level(), projectileStack, hero, bowStack);
            Item var9 = bowStack.getItem();
            if (var9 instanceof BowItem) {
               BowItem bowItem = (BowItem)var9;
            ...
            hero.level().playSound((Player)null, hero.getX(), hero.getY(), hero.getZ(), SoundEvents.ARROW_SHOOT, SoundSource.HOSTILE, 1.0F, 1.0F / (hero.getRandom().nextFloat() * 0.4F + 1.2F) + power * 0.5F);
   ...
   public static boolean fireCrossbow(HeroEntity hero, ItemStack stack) {
      return fireCrossbow(hero, (LivingEntity)null, stack);
   }

   public static boolean fireCrossbow(HeroEntity hero, LivingEntity target, ItemStack stack) {
      return hero != null && target != null && stack != null && !stack.isEmpty() && !hero.level().isClientSide ? fireVirtualProjectileAtTarget(hero, target, stack, CrossbowItem.getChargeDuration(stack, hero), 3.15F, 1.0F, SoundEvents.CROSSBOW_SHOOT) : false;
   }
   ...
   private static void aimAlongShotVector(HeroEntity hero, Vec3 shotVector) {
      ...
      float yaw = (float)(Mth.atan2(shotVector.z, shotVector.x) * (double)(180F / (float)Math.PI)) - 90.0F;
      float pitch = (float)(-(Mth.atan2(shotVector.y, horizontalDistance) * (double)(180F / (float)Math.PI)));
      hero.setYRot(yaw);
      hero.setXRot(pitch);
   ...
   private static Vec3 createProjectileShotVector(AbstractArrow arrow, HeroEntity hero, LivingEntity target, float launchVelocity) {
      ...
      double targetY = target.getEyeY() - Math.max(0.1, (double)target.getBbHeight() * 0.15);
      double baseDy = targetY - arrow.getY();
      if (horizontalDistance < 1.0E-4) {
         return new Vec3(dx, baseDy, dz);
      } else {
         double speed = Math.max(0.1, (double)launchVelocity);
         double speedSq = speed * speed;
         double discriminant = speedSq * speedSq - 0.05 * (0.05 * horizontalDistance * horizontalDistance + (double)2.0F * baseDy * speedSq);
         double adjustedDy;
         if (discriminant >= (double)0.0F) {
            double sqrt = Math.sqrt(discriminant);
            double tanTheta = (speedSq - sqrt) / (0.05 * horizontalDistance);
            adjustedDy = horizontalDistance * tanTheta;
         } else {
            adjustedDy = baseDy + horizontalDistance * 0.2;
         }
   ...
   private static boolean fireVirtualProjectileAtTarget(HeroEntity hero, LivingEntity target, ItemStack weaponStack, int chargeTicks, float velocity, float inaccuracy, SoundEvent shootSound) {
      if (hero != null && target != null && weaponStack != null && !weaponStack.isEmpty() && !hero.level().isClientSide) {
         ...
         float powerScale = weaponStack.getUseAnimation() == UseAnim.BOW ? BowItem.getPowerForTime(Math.max(chargeTicks, 20)) : Mth.clamp((float)chargeTicks / (float)Math.max(1, getChargeTicks(weaponStack, hero)), 0.65F, 1.0F);
         ...
         hero.level().addFreshEntity(arrow);
         hero.level().playSound((Player)null, hero.getX(), hero.getY(), hero.getZ(), shootSound, SoundSource.HOSTILE, 1.0F, 1.0F / (hero.getRandom().nextFloat() * 0.4F + 1.2F) + powerScale * 0.5F);
         hero.swing(InteractionHand.MAIN_HAND);
         return true;
      } else {
         return false;
