...
public final class HeroCombatPlanner {
    ...
    public static ActionPhase getActionPhase(HeroEntity hero) {
        ...
        return getActionProfileForState(hero, actionState).phaseSpec().phaseFor(hero.getBattleActionTicks());
    }
    ...
    public static UtilityScores evaluateUtility(HeroEntity hero, LivingEntity target, CombatTuning tuning) {
        ...
        ActionProfile dashProfile = getActionProfile(PlannedAction.DASH, hero, tuning);
        ActionProfile airProfile = getActionProfile(PlannedAction.AIR, hero, tuning);
        ...
        if (tuning.airAvailable() && tuning.airMaxDistance() > 0.0D) {
            double predictedAirRangeFit = fitMax(Math.sqrt(airProfile.predictedDistanceSqr(hero, target)), tuning.airMaxDistance() + 0.6D);
    ...
    public static boolean shouldPrepareBufferedAction(HeroEntity hero) {
        ...
        return getActionProfileForState(hero, state).phaseSpec().shouldPreBuffer(hero.getBattleActionTicks())
    ...
    public static ActionProfile getActionProfileForState(HeroEntity hero, int actionState) {
        if (hero != null) {
            ...
            if (currentProfile != null && actionState != HeroEntity.BATTLE_ACTION_IDLE && actionState != HeroEntity.BATTLE_ACTION_APPROACH) {
    ...
    public static ActionProfile getActionProfile(PlannedAction action, HeroEntity hero, CombatTuning tuning) {
        return switch (action) {
            ...
            case DASH -> hero != null && hero.getBattleDashProfile() != null ? hero.getBattleDashProfile() : DASH_PROFILE;
            case AIR -> hero != null && hero.getBattleAirProfile() != null ? hero.getBattleAirProfile() : AIR_PROFILE;
