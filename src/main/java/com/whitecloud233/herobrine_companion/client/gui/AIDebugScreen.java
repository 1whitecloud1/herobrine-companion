...
import com.whitecloud233.herobrine_companion.client.service.AIDebugLog;
...
public class AIDebugScreen extends Screen {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        int listRight = 8 + LIST_WIDTH;
        if (mouseX <= listRight) {
            this.listScroll = Math.max(0, this.listScroll - (int) verticalAmount);
        } else {
            this.detailScroll = Math.max(0, this.detailScroll - (int) (verticalAmount * 14));
