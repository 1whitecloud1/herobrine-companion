...
public class ChallengeArenaSliceParticle extends Particle {
    ...
    public void render(VertexConsumer vertex, Camera camera, float partialTicks) {
        ...
        this.drawCap(vertex, cameraPos, yOffset, 0.95F * fade);
        this.drawCap(vertex, cameraPos, yOffset - 0.08F, 0.55F * fade);
    }

    private void drawCap(VertexConsumer vertex, Vec3 cameraPos, float yOffset, float alpha) {
        ...
        for (int i = 1; i < this.capPoints.size() - 1; i++) {
            ...
            this.addVertex(vertex, cameraPos, a, yOffset, alpha);
            this.addVertex(vertex, cameraPos, b, yOffset, alpha);
            this.addVertex(vertex, cameraPos, c, yOffset, alpha);
        }
    }

    private void addVertex(VertexConsumer vertex, Vec3 cameraPos, Vec3 point, float yOffset, float alpha) {
    ...
    public static final ParticleRenderType RENDER_TYPE = new ParticleRenderType() {
        ...
        public BufferBuilder begin(Tesselator tesselator, TextureManager manager) {
            ...
            return tesselator.begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_TEX_COLOR);
        }
