...
public class HeroModel extends PlayerModel<HeroEntity> {
    ...
    // [修正 1] 1.21.1 使用 fromNamespaceAndPath 替代 new ResourceLocation
    ...
    public void setupAnim(HeroEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        // --- 保持原有原版浮空动画逻辑 ---
        // [修正 2] 移除 Minecraft.getInstance() 调用，直接通过 ageInTicks 反推 partialTick
    ...
    private void setupThunderAnim(HeroEntity entity, float ageInTicks) {
        // [修正 2] 同样利用 ageInTicks 计算 partialTick，避开 Minecraft.getInstance()
        float partialTick = ageInTicks - entity.tickCount;
        float progress = entity.getThunderProgress(partialTick);
