...
// 1.21.1 更新：使用 NeoForge 的 EventBusSubscriber，不需要再显式声明 Bus.FORGE
...
public class CleaveTickManager {
    ...
    // 1.21.1 更新：监听明确的 Post 阶段 Tick 事件，代替原先繁琐的 phase 判断
    @SubscribeEvent
    public static void onLevelTick(LevelTickEvent.Post event) {
        if (activeCleaves.isEmpty()) return;

        // 1.21.1 更新：使用 event.getLevel() 获取关卡对象
