...
/** Four native Unity motion sets; V6 and MediaPipe remain registered for review. */
final class PoemScythePlayerAnimations {
    ...
    private static final float SPECIAL_SPEED = 2F;
    ...
    static void registerAnimations(AnimationManager.AnimationBuilder builder) {
        ...
        for (Timing t : timeline.segments) {
            var accessor = builder.<ComboAttackAnimation>nextAccessor(PREFIX + t.name,
                    a -> configure(new ComboAttackAnimation(.10F, a, Armatures.BIPED, phase(t, 0F, COMBO_SPEED)) {
                        @Override
        ...
        var dashAccessor = builder.<DashAttackAnimation>nextAccessor(PREFIX + dash.name,
                a -> configure(new DashAttackAnimation(.10F, a, Armatures.BIPED, phase(dash, 0F, SPECIAL_SPEED)) {
                    @Override
                    public float getPlaySpeed(LivingEntityPatch<?> patch, DynamicAnimation animation) {
        ...
        var airAccessor = builder.<AirSlashAnimation>nextAccessor(PREFIX + air.name,
                a -> configure(new AirSlashAnimation(.08F, a, Armatures.BIPED, phase(air, 0F, SPECIAL_SPEED)) {
                    @Override
                    public float getPlaySpeed(LivingEntityPatch<?> patch, DynamicAnimation animation) {
                        return SPECIAL_SPEED;
                    }
        ...
        airAccessor.get();
        // ComboAttacks reserves the final two entries for sprint and air attacks.
        ...
        builder.<AttackAnimation>nextAccessor(PREFIX + "full", a -> {
            var phases = timeline.segments.stream().map(t -> phase(t, t.start, 1F)).toArray(AttackAnimation.Phase[]::new);
    ...
    static void registerWeaponPreset(WeaponCapabilityPresetRegistryEvent event) {
        event.getTypeEntry().put(WEAPON_TYPE, item -> createCapability());
    }
    ...
    private static WeaponCapability.Builder createCapability() {
        ...
        var capability = WeaponCapability.builder().identifier(WEAPON_TYPE)
                ...
                .swingSound(EpicFightSounds.WHOOSH_BIG).hitSound(EpicFightSounds.BLADE_HIT)
    ...
    private static AttackAnimation.Phase phase(Timing t, float start, float playbackSpeed) {
        // Keep an input window in real seconds when speeding up the source clock.
        // Never open the next attack before the blade's contact interval finishes.
        float recovery = playbackSpeed > 1F
                ? Math.min(t.recovery, Math.max(t.contact, t.duration - COMBO_LINK_WINDOW_SECONDS * playbackSpeed))
                : t.recovery;
