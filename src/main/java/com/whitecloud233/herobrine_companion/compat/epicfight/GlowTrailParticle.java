...
public class GlowTrailParticle extends AnimationTrailParticle {
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(
            ...
            InteractionHand resolvedHand = result.hand();
            if (resolvedHand != null && entitypatch.isMirrorMode() && resolvedHand == InteractionHand.MAIN_HAND) {
                resolvedHand = InteractionHand.OFF_HAND;
            }
            ...
            result = entitypatch.getEntityDecorations()
                    .getModifiedTrailInfo(result, resolvedHand == null ? CapabilityItem.EMPTY : entitypatch.getAdvancedHoldingItemCapability(resolvedHand));
            if (!result.playable()) {
                return null;
            }
            ...
            if (entitypatch.isMirrorMode()) {
                Joint mirroredJoint = entitypatch.getArmature().searchJointByName(PoseMirror.mirrorJointName(trailJoint.getName()));
                if (mirroredJoint != null) {
                    trailJoint = mirroredJoint;
                }
            }
