...
OUT = ROOT/'output/Herobrine_Scythe_Turn_07_EpicFight'
SOURCE = ROOT/'output/Herobrine_Scythe_Turn_07'
...
blade_review = load(SOURCE/'capture/blade_direction_validation.json')
assert blade_review['corrected_frames'] == [75, 365] and blade_review['minimum_flip_degrees'] > 179.
...
assert geometry['full_body_turns_verified'] and geometry['minimum_body_local_shape_rms'] > .08
assert all(m['shaft_direction_span_degrees'] > 140 and m['sweep_path_degrees'] > 170
           and m['maximum_foot_separation_blocks'] > .55 for m in geometry['attack_metrics'])
report = []
for root, version, loader, package, major, filename in [
    ...
    turns = [r for r in load(root/WORK/'engine_validation.json')
             if r['clip'].endswith(('combo_03','combo_04','combo_05','combo_07','combo_08'))]
    assert len(turns) == 10 and all(r['root_rotation_path_degrees'] > 285 for r in turns)
    ...
    target = OUT/f'Herobrine_Companion-0.38-{version}-{loader}-Turn07.jar'
...
for name in ('capture_report.json', 'selection_report.json'):
    shutil.copy2(ROOT/'build/scythe_turn_07'/name, OUT/'reports'/name)
shutil.copy2(ROOT/'build/scythe_turn_07/reviewed_blade_before.json', OUT/'reports/reviewed_blade_before.json')
for path in SOURCE.glob('*.blend'):
    shutil.copy2(path, OUT/path.name)
for path in (SOURCE/'preview').glob('*'):
    if path.name.startswith(('Turn07_', 'Game_Mesh_')) and path.suffix.lower() in ('.mp4', '.jpg'):
        shutil.copy2(path, OUT/path.name)
shutil.copy2(ROOT/'docs/epicfight-poem-turn07.md', OUT/'README_中文.md')
...
print('TURN07_PACKAGE_COMPLETE', json.dumps(report, ensure_ascii=False))
